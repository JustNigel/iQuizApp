...
class ExamRequestController extends Controller
{
            ...
            return $questionnaires->map(function($questionnaire) use ($category, $trainers, $user) {
                return $trainers->map(function($trainer) use ($category, $questionnaire, $user) {
                    ...
                    return (object) [
                        ...
                        'requestStatus' => $existingRequest ? $existingRequest->request_status : null,
    ...
    public function storeRequest(Request $request)
    {
        ...
        $questionnaireId = $request->input('questionnaire_id'); // Get questionnaire_id

        if (!$trainerId) {
            return redirect()->route('category.join')->with('error', 'Unable to find a trainer for this category.');
        }

        // Check if questionnaire_id is not null
    ...
    {
        $examRequest = ExamRequest::where('id', $id)->where('request_status', 'pending')->firstOrFail();
        $examRequest->delete();
