...
<div class="max-w-6xl mx-auto bg-white p-8 rounded-lg shadow-lg">
    <div class="flex justify-between mb-6">
        ...
        @if ($previousPage === 'all-exams')
            <a href="{{ route('trainer.all-confirmed-students') }}" class="text-indigo-600 hover:text-indigo-700 font-medium inline-block">&larr; Return to All Students Exam</a>
        @else
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 text-center whitespace-nowrap text-sm font-medium">
                            <div class="flex justify-center space-x-3">
                                <form action="{{ route('trainer.request.accept', $exam_request->id) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                ...
                                <form action="{{ route('trainer.denyExamRequest', $exam_request->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
