...
<div class="max-w-4xl mx-auto bg-white p-8 rounded-lg shadow-md">
    ...
    @if ($previousPage === 'all-questionnaires')
        <a href="{{ route('trainer.all-questionnaires') }}" class="text-indigo-600 hover:text-indigo-700 font-medium inline-block">&larr; Return to All Questionnaires</a>
    @else   
        <a href="{{ route('trainer.all-category') }}" class="text-indigo-600 hover:text-indigo-700 font-medium inline-block">&larr; Return to All Categories</a>
    @endif

    <h1 class="text-3xl font-semibold mb-6 text-center">Edit Questionnaire</h1>

    <form action="{{ route('trainer.update-questionnaire', $questionnaire->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
